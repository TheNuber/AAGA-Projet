...
/**
 * GirvanNewman
 *  - Implémentation standard de l'algorithme de Girvan–Newman (GN) pour la détection de communautés.
 *  - Stratégie: supprimer itérativement les arêtes les plus "centrales" (betweenness élevée),
 ...
 *   - edgeBetweenness(g): calcule l'intermédiarité des arêtes via Brandes (adapté aux arêtes).
 *   - componentPartition(g): calcule la partition (composantes connexes) courante.
 */
public class GirvanNewman {
    /**
     * run
     *  - Entrée: un graphe SimpleGraph (copié en interne pour ne pas modifier l'original).
     *  - Boucle:
     ...
     *  - Sortie: liste ordonnée des partitions (du graphe initial jusqu'au graphe sans arêtes).
     */
    public static List<Map<String, Integer>> run(SimpleGraph input) {
        ...
        while (g.edgeCount() > 0) {
            ...
            // Suppression des arêtes les plus centrales (gestion des ex-aequo)
            for (Edge e : toRemove) g.removeEdge(e.u, e.v);
            // Partition actuelle (composantes connexes)
    ...
    /**
     * copyOf
     *  - Copie superficielle du graphe (mêmes sommets et arêtes) pour préserver l'entrée.
     */
     ...
     *      * Puis on accumule les dépendances delta[] en dépilant un stack (ordre topologique
    ...
    /**
     * componentPartition
     *  - Calcule les composantes connexes via BFS et attribue un identifiant de communauté
     *    (0, 1, 2, ...) à chaque sommet.
     */
    public static Map<String, Integer> componentPartition(SimpleGraph g) {
        Map<String, Integer> part = new HashMap<>();
        int cid = 0;                      // identifiant de communauté courant
